...
function templateMilestoneRow(milestone = {}) {
  ...
  return `<div class="template-milestone-row"><input required name="milestone_name" value="${safe(milestone.name || "")}" placeholder="Nom du jalon" aria-label="Nom du jalon"><input name="milestone_iteration" type="number" min="1" placeholder="Toutes" value="${milestone.iteration || ""}" aria-label="Iteration cible"><select name="milestone_position" aria-label="Position relative"><option value="first-day" ${position.kind === "first-day" ? "selected" : ""}>Premier jour</option><option value="last-day" ${position.kind === "last-day" ? "selected" : ""}>Dernier jour ouvré</option><option value="day-of-iteration" ${position.kind === "day-of-iteration" ? "selected" : ""}>Jour de l'iteration</option><option value="week-day" ${position.kind === "week-day" ? "selected" : ""}>Semaine / jour</option></select><input name="milestone_week" type="number" min="1" max="52" placeholder="Semaine" value="${position.week || ""}" aria-label="Semaine relative"><input name="milestone_day" type="number" min="1" max="31" placeholder="Jour ISO" value="${position.dayOfWeek || position.day || ""}" aria-label="Jour relatif"><input name="milestone_time" type="time" value="${safe(milestone.time || "")}" aria-label="Heure"><select name="milestone_color" aria-label="Couleur">${colorOptions(milestone.color || "orange")}</select><button type="button" class="icon-btn" data-action="remove-template-milestone" title="Supprimer le jalon" aria-label="Supprimer le jalon">&times;</button></div>`;
}
function templatePeriodPositionOptions(position = {}) {
  return `<option value="first-day" ${position.kind === "first-day" ? "selected" : ""}>Premier jour</option><option value="last-day" ${position.kind === "last-day" ? "selected" : ""}>Dernier jour ouvré</option><option value="day-of-iteration" ${position.kind === "day-of-iteration" ? "selected" : ""}>Jour de l'iteration</option><option value="week-day" ${position.kind === "week-day" ? "selected" : ""}>Semaine / jour</option>`;
}
